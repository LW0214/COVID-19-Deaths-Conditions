...
To further our analysis, we will also be utilizing US population data
obtained from CDC Wonder. CDC provides annual estimates of the
population for each state in the United States, including demographic
breakdowns by age. The CDC obtains population data by integrating
various data sources and applying advanced statistical methodologies.
The primary foundation is decennial census data, which offers a
comprehensive snapshot of the US population. However, given the ten-year
interval between censuses, the CDC supplements this with other data
sources to capture more current population dynamics.
...
Has a value if the number of deaths is \<10. For said entries, "COVID-19
Deaths" and "Number of Mentions" are already suppressed to NAN values to
be in accordance with NCHS confidentiality standards.
...
1.  Impact of Specific Health Conditions on COVID-19 Mortality Rates
    ...
    higher mortality rates within covid-19 patients.
...
2.  Age-Related Vulnerability
    ...
    How: Comparative analysis across age groups: calculating average
    mortality rate per age group or analyzing the frequency and type of
...
3.  Geographical Variation
    ...
    How: Use the 'State' variable to compare covid-19 impacts across
    different jurisdictions. We can apply geographic analysis to
    identify patterns or clusters of high mortality rates.
...
5.  Comparison Between Provisional and Final Data
    ...
    How: Comparative analysis to assess the differences between
...
\"Flag\" column is indicated for such entries. Such entries occupy 30%
of the dataset, which is quite large. We decide to impute the entries
for \"COVID-19 Deaths\"and \"Number of Mentions\" as the average between
0 to 10, which is 5.

At first glance, if an entry\'s column \"Group\" has values \"By
Total\", its \"Year\" and \"Month\" are both missing, because of course
that row is an aggregation of all past data. Similarly, if an entry\'s
column \"Group\" has values \"By Year\", its \"Month\" values are
missing, because that row is an aggregation of all months in that year.
After replacing the fake missing values in these rows with a dummy value
\"-1\", it appears that there are no actual nan values in these columns
anymore. 

Under the column \"Age Group\", there\'s a value \"Not stated\" that
covers 10% of the entire dataset, which isn\'t much, and can be deleted.
