...
1.  Impact of Specific Health Conditions on COVID-19 Mortality Rates
    ...
    How: Analyze 'COVID-19 Deaths' in relation to 'Conditions' and
    'Conditions group' to identify which pre-existing conditions have
    higher mortality rates within covid-19 patients. We can use
    correlation analysis to understand the relationship between specific
    health conditions and mortality rates. Regression analysis can also
    be applied to control for other variables like age.
...
2.  Age-Related Vulnerability
    ...
    How: Conduct age-wise comparison to see how mortality rates differ
    across age groups. Descriptive statistics (like mean, median) and
    graphical representations (like bar charts, age-wise mortality
    curves) would be useful here. We also plan to calculate the average
    mortality rate per age group and analyzing the frequency and type of
    comorbidities in each age group.
...
3.  Geographical Variation
    ...
    How: When analyzing geographical variations, we will only be using
    data in years 2020-2021 (estimates for this year are based on final
    data) as 2022 and 2023 data are provisional. We will do this by
    aggregating the death counts by state for the years 2020-2021 to
...
4.  Trends Over Time
    ...
    How: Time series analysis can be used to track the evolution of the
    pandemic. For example, identifying peaks in mortality rates and
    changes in prevalence of comorbidity over time.

5.  Comparison Between Provisional and Final Data

    Compare the provisional data for 2022 and 2023 with the final data
    for 2020 and 2021. This could reveal insights into the progression
    of the pandemic and the accuracy of provisional data in real-time
    tracking of the pandemic.

    How: Provisional data are not yet complete, counts will not include
    all deaths However, we can estimate how complete the provisional
    numbers are by looking at the average number of deaths reported in
    previous years. This can be done by calculating the percentage of
    completeness of provisional data by comparing it with the historical
    average in 2020 and 2021. For example, if historically 90% of deaths
    are reported within the first 30 days, we can estimate the
    completeness of the provisional data accordingly.
...
# Alaska: sum of known age groups: 249, all ages: 253, 2 age groups unknown
...
is an aggregation of all months in that year. After replacing the fake
missing values in these rows with a dummy value "-1", it appears that
there are no actual nan values in these columns anymore. 
...
# Format US data and add additional columns
us_data_formatted <- us_data %>%
  # Rename 'TotalPopulation' to 'Population'
  rename(Population = TotalPopulation) %>%
  # Add and set new columns to specific values or NA
  mutate(Notes = NA,
         States = "United States",
         `States Code` = 00,
         `Single-Year Ages` = NA,
         `Single-Year Ages Code` = NA,
         `Yearly July 1st Estimates Code` = `Yearly July 1st Estimates`
         )

# Combine US data with original data
all_data <- rbind(us_data_formatted, population_data_formatted) %>%
  filter(is.na(Notes))

# Select and rename relevant columns for the final data
all_data_population <- all_data %>%
  select(States, AgeGroup, Population, `Yearly July 1st Estimates`) %>%
...
# export
write.csv(covid_with_population, "covid_with_population.csv", row.names=FALSE)
```
